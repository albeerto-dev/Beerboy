...
//Values to change
//Tuning parameters
double Kp = 550, Ki = 0.6  , Kd = 0.8; //calcolato KD tramite ziegler nichols
...
bool modifyAutoStep = false;
bool pitch = false;
bool onoffheat = false;
...
void setup() {
  ...
  sensors.setResolution(thermometerAddress, 10);
...
void loop() {
  lcd.clear();
  selectMode(); //first step : selcet the mode >AUTO or >MANUAL >TUNING
}
...
void change() {
  if (digitalRead(UP_PIN) == HIGH or digitalRead(DOWN_PIN) == HIGH) {
    modalita++;
    if (modalita == 3) {
      modalita = 0;
    }
...
void selectMode() {
  ...
  switch (modalita) {
    ...
    case 2: //autotune or heating algorithm
      lcd.setCursor(11, 1);
      lcd.print(">");
      if (select()) {
        lcd.clear();
...
unsigned long tempoAppoggio;
unsigned long scostamento = 0;
unsigned long attuale;
...
int timer_Mash(int tempo) {
  ...
  while (minutes >= 0) {
    ...
    for (seconds; seconds >= 0; seconds--) {
      ...
      attuale = millis(); // tempo attuale 
      compute_Values();
      tempoAppoggio = millis(); //tempo post calcolo valori
      
      scostamento = 1000 - (tempoAppoggio - attuale); //calcolo di differenza
      
      Serial.print("T appoggio : ");
      Serial.println(tempoAppoggio);
      Serial.print("Scostamento : ");
      Serial.println(scostamento);
...
void printLCD_ManualMode() {
  ...
  while (validateManual == false) {
    ...
    lcd.setCursor(9, 0);
    lcd.print(" ");
    if (select()) { //press OK and set a new Setpoint
      validateManual = true;
    }
  }
  delay(1000);
  while (validateManual == true) {
    ...
    lcd.setCursor(12, 1);
    lcd.print("TEMP");
    if (select()) { //press OK and confirm the new Setpoint
...
void coolingStep() {
  if (immersionChiller == true) {
    ...
    while (finished == false) {
      ...
      lcd.setCursor(8, 1);
      lcd.print("C");
      if (actualTemp < pitchTemperature) {
...
void pitchingTemp() {
  while (!pitch) {
    pitchTemperature = constrain(pitchTemperature, 0, 100);
    lcd.clear();
    lcd.print("Pitching");
    lcd.setCursor(0, 1);
    lcd.print("Temp C :");
    lcd.setCursor(11, 1);
    lcd.print(pitchTemperature);
    pitchTempChange();
    if (select()) {
      pitch = true;
    }
...
void pitchTempChange() {
  if (digitalRead(UP_PIN) == HIGH) {
    pitchTemperature = pitchTemperature + 0.5;
  }
  if (digitalRead(DOWN_PIN) == HIGH) {
    pitchTemperature = pitchTemperature - 0.5;
  }
...
void on_off_heating() {
  ...
  */
  ...
  if(Input < Setpoint-0.5  && steadyPoint == false){ //PRIMO RISCALDAMENTO
    digitalWrite(SSR_PIN, HIGH);
  }
  if(Input >= Setpoint-0.5 && steadyPoint == false){ //ARRIVATO A SETPOINT
    steadyPoint = true;
    digitalWrite(SSR_PIN, LOW);
  }
  if(Input <= Setpoint-1 && steadyPoint == true){ //APSETTO UNA VOLTA SETPOINT CHE SCENDA T E POI RIVADO A SETPOINT
    steadyPoint = false;
  }
}

void riscaldamentoIniziale() {
  digitalWrite(SSR_PIN, HIGH);
  stato = 1;
  Serial.println("Riscaldamento");
}

void assestamentoHeat() {
  if (Input >= Setpoint - 0.5) {
    digitalWrite(SSR_PIN, LOW);
    Serial.println("Raffreddamento");
    stato = 2;
  }
}

void assestamentoCold() {
  if (Input <= Setpoint - 1.0) {
    digitalWrite(SSR_PIN, HIGH);
